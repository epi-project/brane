...
pub fn parse_oas_file<P: Into<PathBuf>>(oas_file: P) -> Result<OpenAPI> {
    ...
    match extension.as_str() {
        "yaml" | "yml" => serde_yaml::from_reader(oas_reader).with_context(|| format!("Failed to parse file as OAS: {oas_file:?}")),
        "json" => serde_json::from_reader(oas_reader).with_context(|| format!("Failed to parse file as OAS: {oas_file:?}")),
