...
pub mod tests {
    ...
    async fn test_archive_unarchive(extra_dirs: PathBuf, skip_dir: bool) {
        ...
        let tempdir: TempDir = match TempDir::new() {
            Ok(tempdir) => tempdir,
            Err(err)    => { panic!("Failed to create a temporary directory: {}", err); },
        };
        ...
        let file2: Vec<u8> = {
            ...
            match getrandom(&mut res) {
                Ok(_)    => res,
                Err(err) => { panic!("Failed to genereate random test bytestring: {}", err); },
            }
        };
        ...
        if let Err(err) = archive_async(&source, &tar_path, skip_dir).await { panic!("Failed to archive the source: {}", err); }
        ...
        if let Err(err) = unarchive_async(&tar_path, &target).await { panic!("Failed to unarchive the tarball: {}", err); }
        ...
        for path in [ &target_path1, &target_path2, &target_path3 ] {
            ...
            if !path.exists()  { panic!("File '{}' didn't make the archivation process (not found)", path.display()); }
            if !path.is_file() { panic!("File '{}' didn't make the archivation process (not a file)", path.display()); }

            // Load the file's contents and compare them
            let mut handle: fs::File = match fs::File::open(path) {
                Ok(handle) => handle,
                Err(err)   => { panic!("Failed to open file '{}': {}", path.display(), err); },
            };
    ...
    #[tokio::test]
    async fn test_tarball_with_root() {
        test_archive_unarchive(PathBuf::new(), false).await;
    }

    /// Test if archiving / unarchiving works, skipping the root folder in the result.
    #[tokio::test]
    async fn test_tarball_without_root() {
        test_archive_unarchive(PathBuf::new(), true).await;
    }
    ...
    #[tokio::test]
    async fn test_tarball_with_root_extra_path() {
        test_archive_unarchive("some/extra/folders/lol".into(), false).await;
    }
    ...
    #[tokio::test]
    async fn test_tarball_without_root_extra_path() {
        test_archive_unarchive("some/extra/folders/lol".into(), true).await;
    }
}
...
pub async fn copy_dir_recursively_async(source: impl AsRef<Path>, target: impl AsRef<Path>) -> Result<(), Error> {
    ...
    if !target.is_dir() {
        if target.exists() { return Err(Error::DirNotADir{ what: "target", path: target.into() }); }
    ...
    let entries: tfs::ReadDir = match tfs::read_dir(source).await {
        Ok(entries) => entries,
        Err(err)    => { return Err(Error::DirReadError{ what: "source", path: source.into(), err }); },
    };
    ...
    while let Some((cur_dir, dst_dir, mut entries)) = todo.pop() {
        ...
        while let e = entries.next_entry().await {
            ...
            let e: tfs::DirEntry = match e {
                Ok(Some(e)) => e,
                Ok(None)    => { break; },
                Err(err)    => { return Err(Error::DirEntryReadError{ what: "source", path: cur_dir, entry: i, err }); },
            };
            ...
            if e_path.is_file() {
                ...
                let entries: tfs::ReadDir = match tfs::read_dir(&e_path).await {
                    Ok(entries) => entries,
                    Err(err)    => { return Err(Error::DirReadError{ what: "nested source", path: e_path, err }); },
                };
...
pub async fn archive_async(source: impl AsRef<Path>, tarball: impl AsRef<Path>, skip_root_dir: bool) -> Result<(), Error> {
    ...
    let handle: tfs::File = match tfs::File::create(tarball).await {
        Ok(handle) => handle,
        Err(err)   => { return Err(Error::FileCreateError{ what: "target tarball", path: tarball.into(), err }); },
    };
    ...
    while let Some((path, name)) = todo.pop() {
        ...
        if path.is_file() {
            ...
            let mut entries: tfs::ReadDir = match tfs::read_dir(&path).await {
                Ok(entries) => entries,
                Err(err)    => { return Err(Error::DirReadError{ what: "nested source", path, err }); },
            };
            let mut i: usize = 0;
            loop {
                ...
                let entry: tfs::DirEntry = match entries.next_entry().await {
                    Ok(Some(entry)) => entry,
                    Ok(None)        => { break; },
                    Err(err)        => { return Err(Error::DirEntryReadError { what: "nested source", path, entry: i, err }); },
                };
                i += 1;

                // Compute the tar-side path of this file
                let name: &OsStr = if skip_root_dir && is_root_dir {
                    OsStr::new("")
                } else {
                    &name
                };
    ...
    match tar.into_inner().await {
        Ok(mut enc)  => {
            ...
            if let Err(err) = enc.shutdown().await { return Err(Error::EncoderFlushError{ err }); };
            Ok(())
        },
...
pub async fn unarchive_async(tarball: impl AsRef<Path>, target: impl AsRef<Path>) -> Result<(), Error> {
    ...
    if target.exists() { return Err(Error::PathExistsError{ what: "target", path: target.into() }); }
    if let Err(err) = tfs::create_dir(target).await { return Err(Error::DirCreateError { what: "target", path: target.into(), err }); }

    // Open the source tarfile
    let handle: tfs::File = match tfs::File::open(tarball).await {
        Ok(handle) => handle,
        Err(err)   => { return Err(Error::FileOpenError{ what: "source tarball", path: tarball.into(), err }); },
    };
    ...
    while let Some(entry) = entries.next().await {
        ...
        let entry_path: PathBuf = match entry.path() {
            ...
            Err(err)       => { return Err(Error::TarEntryPathError{ path: tarball.into(), entry: i, err }); },
        };
        ...
        match entry.unpack_in(&target).await {
            Ok(true)  => {},
            Ok(false) => { return Err(Error::PathWithParentDir { what: "nested target", path: target_path }); },
            Err(err)  => { return Err(Error::TarExtractError { tarball: tarball.into(), entry: entry_path, target: target_path, err }); }
        }
